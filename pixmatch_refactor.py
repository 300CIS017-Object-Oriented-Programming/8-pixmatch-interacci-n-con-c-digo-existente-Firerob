...
# Estilos y elementos HTML para la interfaz
...
# Inicialización de la sesión y estado del juego
...
if "GameDetails" not in mystate:
    mystate.GameDetails = ['Medium', 6, 7,
                           '']  # Nivel de dificultad, intervalo en segundos para la generación automática, celdas totales por fila o columna, nombre del jugador


# Función para reducir el espacio desde la parte superior de la página
def ReduceGapFromPageTop(wch_section='main page'):
    if wch_section == 'main page':
        st.markdown(" <style> div[class^='block-container'] { padding-top: 2rem; } </style> ", True)  # Área principal
...
# Función para manejar el leaderboard
def Leaderboard(what_to_do):
    if what_to_do == 'create':
        ...
        if mystate.GameDetails[3] != '':  # Registrar en el leaderboard solo si se proporciona el nombre del jugador
            if os.path.isfile(vpth + 'leaderboard.json'):
                ...
                leaderboard = dict(sorted(leaderboard.items(), key=lambda item: item[1]['HighestScore'],
                                          reverse=True))  # Ordenar descendentemente

                if len(leaderboard) > 4:
                    for i in range(len(leaderboard) - 4): leaderboard.popitem() # Eliminar la última clave del diccionario
...
def InitialPage():
    ...
    # Elige aleatoriamente una imagen para mostrar en la sección de ayuda
    random.seed()
...
def PressedCheck(vcell):
    # Función para verificar si se ha presionado un botón
...
def ResetBoard():
    ...
    if sidebar_emoji_in_list == False:  # La imagen de la barra lateral no está en ningún botón; agregar imagen aleatoriamente
...
def PreNewGame():
    ...
    # Define listas de emojis según la dificultad del juego
    foxes = ['😺', '😸', '😹', '😻', '😼', '😽', '🙀', '😿', '😾']
    emojis = ['😃', '😄', '😁', '😆', '😅', '😂', '🤣', '😊', '😇', '🙂', '🙃', '😉', '😌', '😍', '🥰', '😘', '😗', '😙', '😚', '😋', '😛', '😝', '😜', '🤪', '🤨', '🧐', '🤓', '😎', '🤩', '🥳', '😏', '😒', '😞', '😔', '😟', '😕', '🙁', '☹️', '😣', '😖', '😫', '😩', '🥺', '😢', '😠', '😳', '😥', '😓', '🤗', '🤔', '🤭', '🤫', '🤥', '😶', '😐', '😑', '😬', '🙄', '😯', '😧', '😮', '😲', '🥱', '😴', '🤤', '😪', '😵', '🤐', '🥴', '🤒']
    humans = ['👶', '👧', '🧒', '👦', '👩', '🧑', '👨', '👩‍🦱', '👨‍🦱', '👩‍🦰', '‍👨', '👱', '👩', '👱', '👩‍', '👨‍🦳', '👩‍🦲', '👵', '🧓', '👴', '👲', '👳']
    foods = ['🍏', '🍎', '🍐', '🍊', '🍋', '🍌', '🍉', '🍇', '🍓', '🍈', '🍒', '🍑', '🥭', '🍍', '🥥', '🥝', '🍅', '🍆', '🥑', '🥦', '🥬', '🥒', '🌽', '🥕', '🧄', '🧅', '🥔', '🍠', '🥐', '🥯', '🍞', '🥖', '🥨', '🧀', '🥚', '🍳', '🧈', '🥞', '🧇', '🥓', '🥩', '🍗', '🍖', '🦴', '🌭', '🍔', '🍟', '🍕']
...
def NewGame():
    ...
    for vcell in range(1, (total_cells_per_row_or_col ** 2) + 1):
        ...
        # Resto de las condiciones para cada fila del tablero
...
def Main():
    # Reduce el ancho de la barra lateral y establece el color de los botones
    ...
    with st.sidebar:
        # Configuración de la dificultad y nombre del jugador
...
# Si 'runpage' no está en mystate, establece la página de ejecución como Main
